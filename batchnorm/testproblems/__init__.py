...
import numpy as np
import torch
from torch.nn.utils.weight_norm import remove_weight_norm as rwn
import deepobs
...
class ObsExtPTRunner(deepobs.pytorch.runners.runner.PTRunner):
    ...
    Second on includes position argument for batch norm layers
    """
    ...
    @staticmethod
    def create_BNtestproblem(testproblem, batch_size, position, l2_reg, random_seed):
        """Sets up the deepobs.pytorch.testproblems.testproblem instance.
        Args:
            testproblem (str): The name of the testproblem.
            batch_size (int): Batch size that is used for training
            position (array): Position of the BN layers
            l2_reg (float): Regularization factor
            random_seed (int): The random seed of the framework
        Returns:
            deepobs.pytorch.testproblems.testproblem: An instance of
        ...
        # if the user specified L2-regularization, use that one
        if l2_reg is not None:
            tproblem = testproblem_cls(batch_size, position, l2_reg)
        # else use the default of the testproblem
        else:
            tproblem = testproblem_cls(batch_size, position)
...
# # OBS UTILS
# #
# # seperate function for BN model
# ##############################################################################
def get_obs_testproblem(tproblem_name, batch_size=32, seed=0,
                        runner_class=ObsExtPTRunner):
    """
    Set-up OBS problem.
    :param tproblem_name: A subset of ``cifar100_3c3d, cifar100_allcnnc,
      ...
      svhn_3c3d`` or our custom problems.
...
def get_obs_BNtestproblem(tproblem_name, batch_size=32, position=[0,0,0,0,0], seed=0,
                        runner_class=ObsExtPTRunner):
    """
    Set-up OBS problem.
    :param tproblem_name: A subset of ``cifar100_3c3d, cifar100_allcnnc,
    ...
    # Create DeepOBS testproblem
    tproblem = runner_class.create_BNtestproblem(
        testproblem=tproblem_name,
        batch_size=batch_size,
        position=position,
        l2_reg=0,
